...
func setupTestQueueRunner() (*QueueRunner, *claude.MockExecutor, *bytes.Buffer) {
	runner, mockExecutor, buf := setupTestRunner()
	queueRunner := NewQueueRunner(runner)
	return queueRunner, mockExecutor, buf
}
...
func TestQueueRunner_RunQueue_SingleStory_Success(t *testing.T) {
	queueRunner, mockExecutor, _ := setupTestQueueRunner()

	ctx := context.Background()
	exitCode := queueRunner.RunQueue(ctx, []string{"story-1"})
...
func TestQueueRunner_RunQueue_MultipleStories_Success(t *testing.T) {
	queueRunner, mockExecutor, _ := setupTestQueueRunner()

	ctx := context.Background()
	exitCode := queueRunner.RunQueue(ctx, []string{"story-1", "story-2", "story-3"})

	assert.Equal(t, 0, exitCode)
	// Should have 12 prompts (4 steps × 3 stories)
	assert.Len(t, mockExecutor.RecordedPrompts, 12)
}

func TestQueueRunner_RunQueue_FailsAtFirstStory(t *testing.T) {
	buf := &bytes.Buffer{}
	printer := output.NewPrinterWithWriter(buf)
	cfg := config.DefaultConfig()
	...
	// Override executor to fail on second call (during first story)
	failingExecutor := &failOnNthCallExecutor{
		inner:   mockExecutor,
		failOn:  2,
		current: &callCount,
	...
	ctx := context.Background()
	exitCode := queueRunner.RunQueue(ctx, []string{"story-1", "story-2"})

	assert.Equal(t, 1, exitCode)
	// Should have stopped before completing all stories
	assert.Less(t, callCount, 8) // Less than 4 steps × 2 stories
}

func TestQueueRunner_RunQueue_FailsAtSecondStory(t *testing.T) {
	buf := &bytes.Buffer{}
	printer := output.NewPrinterWithWriter(buf)
	cfg := config.DefaultConfig()
	...
	// Override executor to fail on 5th call (first call of second story)
	failingExecutor := &failOnNthCallExecutor{
		inner:   mockExecutor,
		failOn:  5,
		current: &callCount,
	...
	assert.Equal(t, 1, exitCode)
	// First story (4 calls) + 1 failed call = 5 calls
	assert.Equal(t, 5, callCount)
}

func TestQueueRunner_runFullCycleInternal_Success(t *testing.T) {
	queueRunner, mockExecutor, _ := setupTestQueueRunner()

	ctx := context.Background()
	exitCode := queueRunner.runFullCycleInternal(ctx, "test-story")

	assert.Equal(t, 0, exitCode)
	assert.Len(t, mockExecutor.RecordedPrompts, 4)
}

func TestQueueRunner_runFullCycleInternal_FailsAtStep(t *testing.T) {
	buf := &bytes.Buffer{}
	printer := output.NewPrinterWithWriter(buf)
	cfg := config.DefaultConfig()
	...
	// Override executor to fail on third call
	failingExecutor := &failOnNthCallExecutor{
		inner:   mockExecutor,
		failOn:  3,
		current: &callCount,
